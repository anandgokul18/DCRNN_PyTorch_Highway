...
class DCRNNSupervisor:
    def __init__(self, adj_mx, args, **kwargs):
        self._kwargs = kwargs
    ...
    def _train(self, args, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            #Adding this before for Deepspeed
            if batches_seen == 0:
                    # this is a workaround to accommodate dynamically registered parameters in DCGRUCell
                    optimizer = torch.optim.Adam(self.dcrnn_model.parameters(), lr=base_lr, eps=epsilon)

            #Deepspeed model engine initialization
            model_engine, optimizer, trainloader, __ = deepspeed.initialize(args=args, model=self.dcrnn_model, model_parameters=self.dcrnn_model.parameters(), optimizer=optimizer)

            #Deepspeed model training
            for _, (x, y) in enumerate(train_iterator):

                #Zeroing the gradients is automatically handled by Deepspeed

                #Get input, label
                x, y = self._prepare_data(x, y)

                #Forward pass
                output = model_engine(x,y,batches_seen)
